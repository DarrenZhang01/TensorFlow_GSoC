...
import sys
sys.stdout = open("../run_test_v1.txt", 'w')
...
class StaxTest(jtu.JaxTestCase):
  ...
  def testRandnInitShape(self, shape):
    ...
    # Small revision (2020.06.22): original `stax.randn` support is from JAX nn
    # normal initializers in stax file. Since now all the JAX initializers have
    # been replaced by TF initializers, in order to use this API for testing
    # purposes, I directly import normal in the test file.
    out = normal()(key, shape)
