...
def _get_inputs(
  ...
  x1 = fn(np.asarray(normal(shape, seed=key)))
  batch_axis = shape.index(BATCH_SIZE)
  shape = shape[:batch_axis] + (2 * BATCH_SIZE,) + shape[batch_axis + 1:]
  x2 = None if same_inputs else fn(np.asarray(normal(shape, seed=split))) * 2
  return x1, x2
...
class StaxTest(test_utils.NeuralTangentsTestCase):
  ...
  #   x_dense = np.asarray(normal((input_count, input_size), seed=key))
  #   indices = [[dim] for dim in list(range(sparse_count))]
  #   x_sparse = tf.tensor_scatter_nd_update(x_dense, np.array(indices), np.zeros([sparse_count, input_size]))
  ...
  #   # tf.print("x_sparse: {}".format(x_sparse), output_stream=sys.stderr)
  ...
  #   x1 = np.asarray(normal((10, 10), seed=rng))
  #   x2 = np.asarray(normal((10, 10), seed=rng))
  #
  ...
  #   rng = stateless_uniform(shape=[2], seed=[0, 0], minval=None, maxval=None, dtype=tf.int32)
  #   x1 = np.asarray(normal((5, 10, 10, 3), seed=rng))
  #   x2 = None if same_inputs else np.asarray(normal((5, 10, 10, 3), seed=rng))
  #
...
#     key = stateless_uniform(shape=[2], seed=[1, 1], minval=None, maxval=None, dtype=tf.int32)
#     X0_1 = np.asarray(normal((5, 7), seed=key))
...
#
#     X0_2 = None if same_inputs else np.asarray(normal((9, 7), seed=key))
#
...
#                            'NC' + spatial_spec)
#       X0_1 = np.asarray(normal((2, 3) + spatial_shape, seed=key))
#       X0_2 = None if same_inputs else np.asarray(normal((4, 3) + spatial_shape, seed=key))
#     else:
#       channel_axis = -1
#       dimension_numbers = ('N' + spatial_spec + 'C', filter_spec,
#                            'N' + spatial_spec + 'C')
#       X0_1 = np.asarray(normal((2,) + spatial_shape + (3,), seed=key))
#       X0_2 = None if same_inputs else np.asarray(normal((4,) + spatial_shape + (3,), seed=key))
#
...
#     key = stateless_uniform(shape=[2], seed=[1, 1], minval=None, maxval=None, dtype=tf.int32)
#     x1 = np.asarray(normal((2, 5, 6, 3), seed=key))
#     x2 = None if same_inputs else np.asarray(normal((3, 5, 6, 3), seed=key))
#
...
#
#     key = stateless_uniform(shape=[2], seed=[1, 1], minval=None, maxval=None, dtype=tf.int32)
#     x1 = np.asarray(normal((2, 7, 8, 4, 3), seed=key))
#     x2 = None if same_inputs else np.asarray(normal((4, 7, 8, 4, 3), seed=key))
#
...
#                   p,
#           }
#           for same_inputs in [True, False] for get in ['ntk', 'nngp']
...
#         key_ = [key, key] if key.shape == [] else key
#         mask = stateless_uniform(shape=mask_shape, seed=key_, minval=0, maxval=1) < p
#         x = np.where(mask, mask_constant, x)
#       return x
#
#     x1 = np.asarray(normal((4, 6, 5, 7), seed=key))
#     x1 = apply_mask(x1)
#
#     if same_inputs:
#       x2 = None
#     else:
#       x2 = np.asarray(normal((2, 6, 5, 7), seed=key))
#       x2 = apply_mask(x2)
#
#     nn = stax.serial(
#         stax.Flatten(),
...
#       return x
#
#     x1 = np.asarray(normal((4,) + spatial_shape + (3,), seed=key))
#     x1 = apply_mask(x1)
#
#     if same_inputs:
#       x2 = None
#     else:
#       x2 = np.asarray(normal((2,) + spatial_shape + (3,), seed=key))
#       x2 = apply_mask(x2)
#
#     def get_attn():
